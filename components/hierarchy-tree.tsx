      ...
      <div className="flex items-stretch gap-2">
        ...
        <button
          ...
          className={`min-w-0 flex-1 rounded border p-3 text-left transition ${
            isSelected ? "border-cinnabar bg-cloud shadow-sm" : "border-black/10 bg-white hover:bg-cloud"
          }`}
        >
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <Network className="size-5 text-cinnabar" />
            <p className="text-sm font-semibold uppercase tracking-[0.16em] text-cinnabar">Hierarchy Tree</p>
          </div>
        ...
        <div className="flex gap-2">
          ...
          <button className="flex items-center gap-2 rounded bg-ink px-3 py-2 text-sm font-semibold text-white">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {downlineSummary.map((item) => (
          <div key={item.label} className="rounded border border-black/10 bg-rice p-4">
            ...
            <p className="mt-1 text-xs text-cinnabar">{item.helper}</p>
          </div>
        ))}
      </div>
            ...
            <div className="mt-3 max-h-72 overflow-y-auto scrollbar-soft">
              {filteredMembers.map((member) => (
                ...
                >
                  ...
                  <span className="shrink-0 text-sm font-semibold text-cinnabar">{member.commission}</span>
                </button>
              ))}
            </div>
          </div>
