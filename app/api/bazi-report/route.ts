...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await client.responses.create({
      ...
      instructions:
        "你是易玺老师的八字命理报告助理，精通四柱八字、十神、五行强弱、合冲刑害、大运流年，并能把术语翻译成用户能执行的现实建议。必须严格采用系统提供的 analysis JSON，不得自行发明四柱、农历或干支。输出必须是 JSON，格式为 {\"summary\":\"...\",\"sections\":[{\"title\":\"...\",\"content\":\"...\"}]}。sections 需要 8-10 个，每段必须包含【盘面依据】【白话判断】【风险提醒】【行动建议】中的至少两个标签。单段不要超过 180 字。不要声称绝对准确，不要提供金融、法律、医疗或专业建议。",
      input: `
用户资料：
