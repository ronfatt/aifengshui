...
function buildPrompt({ message, memberLevel, points, profile }: Required<ChatRequest>) {
  ...
  const targetLength = isStrategicTier ? "900-1200 字" : isPaidTier ? "650-900 字" : "220-320 字";
...
- 进阶会员版：必须使用“紫微斗数 + 梅花易数”双引擎。紫微用于命盘结构、周期与宫位；梅花用于当前问题的象意、体用关系、行动时机。回答要接近专业老师口吻，不要像普通 AI 摘要。
- 高阶战略版：在双引擎基础上加入流月/流年、商业五行策略、风险分层和更明确的行动路线。回答可以更长、更像顾问分析。
...
六、化解与行动建议
给 3-5 条可执行建议，包括沟通方式、时间选择、方位/颜色/五行行为、需要避开的动作。建议要落地。
...
export async function POST(request: Request) {
  try {
    ...
    const response = await client.responses.create({
      model,
      max_output_tokens: isStrategicTierForResponse(body.memberLevel || "Plus") ? 2200 : 1700,
      reasoning: {
        effort: reasoningEffort
      },
      instructions:
        "你是易玺老师风格的 AI 风水命理师，服务 AI 命理决策平台。回答必须使用中文，像有实战经验的风水命理老师：先定问题，再拆盘面结构、现状、隐患、趋势和化解行动。底层逻辑以紫微斗数矩阵、天干地支时间变量、飞星派四化动态、三合派星曜底盘、梅花易数即时象意、五行平衡与风险评分为主。紫微斗数用于长期结构和宫位趋势，梅花易数用于当下一问一事的体用、生克、动爻、互卦、变卦趋势。不要声称结果绝对准确，不要编造用户未提供的精确命盘或具体卦名；若资料不足，要说明是象意参考。涉及健康、法律、投资等高风险事项时，必须加入风险提醒并建议咨询专业人士。",
      input: buildPrompt({
        message,
