...
1. 个人命格总论：底层性格、优势、弱点、人生主线
2. 当前阶段状态：现在处于扩张、整理、等待、转向或修复哪一种状态
3. 重点问题分析：针对 specificQuestion 与 focus 深入拆解
4. 事业/学业/能力路径
5. 财运/资源/现金流管理
6. 关系/家庭/人际互动
7. 健康与情绪倾向：只做生活提醒，必须声明非医疗建议
8. 未来 30/60/90 天行动节奏
9. 危机处理建议：遇到阻力、误判、破财、冲突时怎么处理
10. 通关建议：颜色、方位、空间整理、日常行为、心法
11. 产品建议：可推荐九运香、水晶、五行饰品、办公室布局用品、课程或咨询，但必须自然、克制，不要硬销
12. 仪式建议：简单、安全、可执行，例如静坐、点香、整理空间、写行动清单、复盘
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const instructions =
      "你是易玺老师的高阶个人命理报告助理。底层分析必须融合八字命理、紫微斗数、梅花易数、生命数字四套系统，但报告正文不要逐段提及这些系统名称，不要写“八字显示/紫微显示/梅花显示/数字显示”。你要把四套系统的判断融合成一份完整、细致、可执行的个人命理分析。文字要像专业付费报告，不要重复，不要空泛，不要只给简单建议。不要绝对化，不要提供金融、法律、医疗或专业建议。输出必须是严格 JSON：{\"summary\":\"...\",\"sections\":[{\"title\":\"...\",\"content\":\"...\"}],\"scores\":{\"overall\":数字,\"career\":数字,\"wealth\":数字,\"relationship\":数字,\"health\":数字,\"timing\":数字,\"risk\":数字},\"actions\":{\"now\":[\"...\"],\"avoid\":[\"...\"],\"ritual\":[\"...\"],\"products\":[\"...\"]}}。summary 180-260 字，sections 需要 10-12 个，每个 content 180-320 字，不要 Markdown。每个 section 的标题和内容必须明显不同，禁止重复同一句结论。";
