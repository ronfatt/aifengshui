...
export default function AuthPage() {
  ...
  return (
    <AppShell>
      <main className="bg-[#DDEFF2] px-5 py-12">
        <section className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[2rem] bg-[#063F4A] p-8 text-white shadow-soft">
            <p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#C79A54]">PUBLIC ALMANAC</p>
            <h1 className="mt-4 text-4xl font-semibold leading-tight">大众每日通胜</h1>
            <p className="mt-5 text-sm leading-7 text-white/65">
              这是公开版今日参考，结合通胜宜忌、五行节气与方位能量，提供大众可用的吉时、吉方与行动提醒。
            </p>

            <div className="mt-8 rounded-3xl border border-[#C79A54]/25 bg-white/8 p-5">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#C79A54]">大众今日总览</p>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                {[
                  ...
                  <div key={label} className="rounded-2xl border border-white/10 bg-white/8 p-4">
                    <div className="flex items-center gap-2 text-xs font-semibold text-[#C79A54]">
            ...
            <div className="mt-5 grid gap-3">
              <div className="rounded border border-[#C79A54]/25 bg-[#C79A54]/10 p-3 text-sm font-semibold leading-6 text-[#E8D4A8]">
              ...
              {[
                ...
                <div key={item} className="flex items-start gap-2 rounded border border-white/10 bg-white/8 p-3 text-sm leading-6 text-white/78">
                  <BadgeCheck className="mt-0.5 size-4 shrink-0 text-[#C79A54]" />
                  {item}
                </div>
              ))}
            </div>
          ...
          <form onSubmit={handleSubmit} className="rounded-[2rem] border border-black/10 bg-white p-6 shadow-sm">
            <div className="flex rounded-full bg-[#F5FAFA] p-1">
              {[
                ["register", "注册"],
                ["login", "登录"]
              ].map(([value, label]) => (
                <button
                  ...
                  className={`flex-1 rounded-full px-4 py-2 text-sm font-semibold ${mode === value ? "bg-[#063F4A] text-white" : "text-ink/58"}`}
                >
                  {label}
                </button>
              ))}
            ...
            <div className="mt-6 grid gap-4">
              {mode === "register" ? (
                <>
                  <label className="grid gap-2 text-sm font-semibold">
                    名字
                    <input value={name} onChange={(event) => setName(event.target.value)} className="rounded border border-black/10 bg-rice px-4 py-3 font-normal outline-none focus:border-[#063F4A]" required />
                  </label>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <label className="grid gap-2 text-sm font-semibold">
                      性别
                      <select value={gender} onChange={(event) => setGender(event.target.value)} className="rounded border border-black/10 bg-rice px-4 py-3 font-normal outline-none focus:border-[#063F4A]" required>
                        <option>男</option>
                        <option>女</option>
                        <option>其他</option>
                      </select>
              ...
              <label className="grid gap-2 text-sm font-semibold">
                Email
                <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="rounded border border-black/10 bg-rice px-4 py-3 font-normal outline-none focus:border-[#063F4A]" required />
              </label>
              <label className="grid gap-2 text-sm font-semibold">
                密码
            ...
            {message ? <p className="mt-5 rounded bg-[#F5FAFA] p-3 text-sm leading-6 text-ink/65">{message}</p> : null}

            <button
              type="submit"
              disabled={isLoading}
              className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded bg-[#1495A0] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#0F7F88] disabled:opacity-60"
            >
